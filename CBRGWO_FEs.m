...
% Grey Wolf Optimizer
function [constraint,Alpha_score,Alpha_pos,Max_FEs,FEs_curve,Convergence_curve]=CBRGWO_FEs(SearchAgents_no,MaxFEs,lb,ub,dim,fobj,subject2)
...
FEs=0;
Max_FEs=FEs;
FEs_curve=zeros(1,MaxFEs);

for i=1:size(Positions, 1)
    % fitness=fobj(Positions(i,:));
    [Positions(i,:),y]=subject2(Positions(i,:));
    if all(y<=0)
        fitness=fobj(Positions(i,:));
    else
        fitness=10^10;
...
% while l<Max_iter
    ...
    for i=1:size(Positions,1)
        ...
        Flag4ub=Seed1>ub;
        Flag4lb=Seed1<lb;
        Seed1=Seed1.*(~(Flag4ub+Flag4lb))+ub.*Flag4ub+lb.*Flag4lb; 
        %F_Seed1 = fobj(Seed1);
        [Seed1,y]=subject2(Seed1);
        if all(y<=0)
            F_Seed1=fobj(Seed1);
        else
            F_Seed1=10^10;
        ...
        Flag4ub=Seed2>ub;
        Flag4lb=Seed2<lb;
        Seed2=Seed2.*(~(Flag4ub+Flag4lb))+ub.*Flag4ub+lb.*Flag4lb; 
        %F_Seed2 = fobj(Seed2);
        [Seed1,y]=subject2(Seed2);
        if all(y<=0)
            F_Seed2=fobj(Seed2);
        else
            F_Seed2=10^10;
        ...
        Flag4ub=Seed3>ub;
        Flag4lb=Seed3<lb;
        Seed3=Seed3.*(~(Flag4ub+Flag4lb))+ub.*Flag4ub+lb.*Flag4lb; 
        %F_Seed3 = fobj(Seed3);
        [Seed3,y]=subject2(Seed3);
        if all(y<=0)
            F_Seed3=fobj(Seed3);
        else
            F_Seed3=10^10;
